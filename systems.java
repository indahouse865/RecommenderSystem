...
public class systems {
	...
	//Mapper Class
	public static class MapperSys extends Mapper<LongWritable, Text, LongWritable, friendCounts> {
	...
	public static class ReducerSys extends Reducer<LongWritable, friendCounts, LongWritable, Text> {
		public void reduce(LongWritable key, Iterable<friendCounts> values, Context context)
			...
			java.util.SortedMap<Long, List<Long>> sortFriends = new TreeMap<Long, List<Long>>(new Comparator<Long>() {
				public int compare(Long key1, Long value) {
					Integer value1 = mutualFriends.get(key1).size();
					Integer value2 = mutualFriends.get(value).size();
					if (value1 > value2) {
						return -1;
					} else if (value1.equals(value2) && key1 < value) {
						return -1;
					} else {
						return 1;
					}
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "systems");
		job.setJarByClass(systems.class);
		job.setOutputKeyClass(LongWritable.class);
		job.setOutputValueClass(friendCounts.class);
		job.setMapperClass(MapperSys.class);
		job.setReducerClass(ReducerSys.class);
